...
const Card:FC<IProps> = ({card, classes}) => {
    ...
    return (
        <div className={`portfolio-card __anim-item ${classes ? classes.join(' ') : ''}`} id={card.id}>
            <div className="portfolio-card__left">
                ...
                <div className="portfolio-card__links ">
                    <div className="anim-show_left delay_06s">
                        <a href={card.url} className="link"><span>перейти</span></a>
                    </div>
                    {
                        card.urlGit && location.pathname !== '/' ? (
                            <div className="anim-show_left delay_06s">
                                <a href={card.urlGit} className="link"><span>GitHub</span></a>
                            </div>
                        ) : null
                    }

                    {
                        location.pathname === '/' ? (
                            <div className="anim-show_left delay_08s">
                                <Link to="/works" className="link "><span>больше</span></Link>
                            </div>
                        ) : null
                    }
                </div>
